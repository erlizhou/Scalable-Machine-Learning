...
def parsePoint(line):
    ...
    """
    split = line.split(',')
    return LabeledPoint(split[0], split[1:])
...
averageTrainYear = (parsedTrainData
                    .map(lambda a: a.label)).sum() / parsedTrainData.count()
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return (labelsAndPreds.map(lambda a: squaredError(a[0], a[1])).sum() / labelsAndPreds.count()) ** 0.5
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        gradient = trainData.map(lambda a: gradientSummand(w, a)).sum()
...
labelsAndPreds = parsedValData.map(lambda a: (a.label, averageTrainYear))
...
def twoWayInteractions(lp):
    ...
    """
    features = lp.features.tolist()
    for i in range(len(lp.features)):
        for j in range(len(lp.features)):
            features.append(lp.features[i] * lp.features[j])
