...
uniqueHostCount = len(uniqueHosts.countByValue())
...
# TODO: Replace <FILL IN> with appropriate code
daysWithHosts = dailyHosts.sortByKey().map(lambda (a, b): a).collect()
hosts = dailyHosts.sortByKey().map(lambda (a, b): b).collect()
...
groupedByDay = dayToHostPairTuple.groupByKey().mapValues(list)

dayRequestCount = groupedByDay.map(lambda (a, b): (a, len(b)))

dayRequest = dayHostCount.join(dayRequestCount)

avgDailyReqPerHost = dayRequest.map(lambda (a, (b, c)): (a, c / b)).sortBy(lambda x: x[0]).cache()
...
errDateCountPairTuple = badRecords.map(lambda hosts: (hosts.date_time.day, 1))
...
hourCountPairTuple = badRecords.map(lambda hosts: (hosts.date_time.hour, 1))
